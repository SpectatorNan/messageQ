package storage

import (
	"os"
	"path/filepath"
	"testing"
	"time"
)

func writeMessages(store *WALStorage, topic string, count int, bodySize int) error {
	for i := 0; i < count; i++ {
		msg := Message{
			ID:        int64(i + 1),
			Body:      string(make([]byte, bodySize)),
			Retry:     0,
			Timestamp: time.Now(),
		}
		if err := store.Append(topic, msg); err != nil {
			return err
		}
	}
	return nil
}

func walFileSize(dir string, topic string) (int64, error) {
	p := filepath.Join(dir, topic+".wal")
	s, err := os.Stat(p)
	if err != nil {
		if os.IsNotExist(err) {
			return 0, nil
		}
		return 0, err
	}
	return s.Size(), nil
}

func cleanupDir(dir string) error {
	return os.RemoveAll(dir)
}

func BenchmarkWAL_Intervals(b *testing.B) {
	cases := []struct {
		name  string
		flush time.Duration
	}{
		{"sync", 0},
		{"10ms", 10 * time.Millisecond},
		{"100ms", 100 * time.Millisecond},
	}

	for _, c := range cases {
		b.Run(c.name, func(b *testing.B) {
			dir := filepath.Join("./benchdata", c.name)
			_ = cleanupDir(dir)
			defer cleanupDir(dir)
			store := NewWALStorage(dir, c.flush, 10*time.Minute)
			defer store.Close()

			// choose workload
			count := 1000
			bodySize := 128 // bytes

			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				if err := writeMessages(store, "bench", count, bodySize); err != nil {
					b.Fatalf("write failed: %v", err)
				}
			}
			b.StopTimer()
			_ = store.Flush()
			sz, _ := walFileSize(dir, "bench")
			b.ReportMetric(float64(sz), "wal_bytes")
		})
	}
}
